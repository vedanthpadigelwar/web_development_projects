...
class MnglOptionsHelper
{
  ...
  public static function display_field_visibility_buttons($field_name, $custom=false, $password=false)
  {
    ...
    MnglOptionsHelper::render_array_checkbox( $mngl_options->field_visibility_str,
                                              (isset($profile_front_fields[$field_name]))?$profile_front_fields[$field_name]:'',
                                              'profile_front',
                                              $field_name, 
                                              $custom,
                                              $password);
    ...
    MnglOptionsHelper::render_array_checkbox( $mngl_options->field_visibility_str,
                                              (isset($profile_info_fields[$field_name]))?$profile_info_fields[$field_name]:'',
                                              'profile_info',
                                              $field_name,
                                              $custom,
                                              $password);
    ...
    MnglOptionsHelper::render_array_checkbox( $mngl_options->field_visibility_str,
                                              (isset($profile_edit_fields[$field_name]))?$profile_edit_fields[$field_name]:'',
                                              'profile_edit',
                                              $field_name,
                                              $custom);
    ...
    MnglOptionsHelper::render_array_checkbox( $mngl_options->field_visibility_str,
                                              (isset($signup_page_fields[$field_name]))?$signup_page_fields[$field_name]:'',
                                              'signup_page',
                                              $field_name,
                                              $custom );
    ...
    ?>
      <select name="<?php echo $name; ?>" id="<?php echo $id; ?>" class="mngl-dropdown mngl-mailer-dropdown" onchange="javascript:mngl_mailer_options();">
        <?php foreach( $options as $opt_key => $opt_value ) { ?>
          <option value="<?php echo $opt_key; ?>" <?php  echo (((isset($_POST[$opt_key]) and $_POST[$opt_key] == $opt_key) or (!isset($_POST[$opt_key]) and $field_value == $opt_key ))?' selected="selected"':''); ?>><?php echo $opt_value; ?>&nbsp;</option>
        <?php } ?>
      </select>
    <?php
  }
