...
<p><a href="javascript:mngl_toggle_message_composer()" class="msg-btn" id="mngl_message_composer_button"><?php _e('Compose a Message', 'mingle'); ?></a>
...
<p>
  ...
  <input type="submit" class="mngl-share-button submit-btn" style="height: 24px; padding: 0px 2px 24px 2px !important;" value="<?php _e('Apply', 'mingle'); ?>" name="mngl_message_action_button" id="mngl_message_action_button" onclick="javascript:mngl_bulk_action()" />
</p>
<?php
if($prev_page > 0)
{
...
?>
<table cellspacing="0" cellpadding="0" class="mngl_messages_table msg-table">
<?php
...
if(is_array($messages) and !empty($messages))
{
  ...
  ?>
    <tr id="mngl_thread_<?php echo $message['thread']->id; ?>" class="mngl_message_row<?php echo (($message['latest']->unread)?" mngl_message_unread":''); ?>">
      ...
      <td style="width: 120px;"><p class="mngl_message_listing"><a href="<?php echo $author->get_profile_url(); ?>"><?php echo "{$author->first_name} {$author->last_name}"; ?></a><br/><span class="mngl_small_gray"><?php echo date('F j \a\t g:ia', $message['latest']->created_at_ts); ?></span></p>
      </td>
