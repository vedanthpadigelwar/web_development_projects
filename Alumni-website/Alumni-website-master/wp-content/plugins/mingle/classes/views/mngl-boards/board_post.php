...
$author_profile_link = "<a href=\"{$author_profile_url}\">{$author->first_name} {$author->last_name}</a>";
$owner_profile_link  = "<a href=\"{$owner_profile_url}\">{$owner->first_name} {$owner->last_name}</a>";
...
?>
  <div class="mngl-board-post-<?php echo $board_post->id; ?> mngl-board-post mngl-board-activity">
    &nbsp;<?php echo stripslashes($message); ?> - <span class="mngl-board-post-second-row"><span class="mngl-time-ago"><?php echo $mngl_app_helper->time_ago($board_post->created_at_ts); ?></span><?php
    if(MnglUser::is_logged_in_and_visible() and ( $mngl_friend->is_friend( $board_post->owner_id, $author_id ) or $mngl_friend->is_self( $board_post->owner_id, $author_id ) ) )
    {
    ?> - <a href="javascript:mngl_toggle_comment_form('<?php echo $board_post->id; ?>')"><?php _e('Comment', 'mingle'); ?></a><?php
    }
...
?>
    <div id="mngl-board-comment-list-<?php echo $board_post->id; ?>" class="mngl-board-comment-list<?php echo ((count($board_post->comments) >= 1)?"":" "); ?>">
    <?php
