...
?>
  <tr class="mngl-friend-request" id="request-<?php echo $request->id; ?>">
    ...
    <td valign="top" style="padding: 0px 0px 0px 10px; vertical-align: top;"><?php printf(__('%s wants to be your friend.', 'mingle'), '<a class="req-name" href="' . $requestor->get_profile_url() . '">' . $requestor->first_name.' '.$requestor->last_name . '</a>'); ?><br/><a class="accept-btn" href="javascript:mngl_accept_friend_request( '<?php echo MNGL_SCRIPT_URL; ?>', <?php echo $request->id; ?>, '<?php echo $requestor->screenname; ?>' )"><?php _e('Accept', 'mingle'); ?></a>&nbsp;|&nbsp;<a class="ignore-btn" href="javascript:mngl_ignore_friend_request( '<?php echo MNGL_SCRIPT_URL; ?>', <?php echo $request->id; ?> )"><?php _e('Ignore', 'mingle'); ?></a></td>
  </tr>
<?php
      }
    }
